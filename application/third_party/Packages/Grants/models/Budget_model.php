...
class Budget_model extends MY_Model
{
  ...
  function replicate_budget($post_array,$approval_id,$header_id){
    ...
    $immediate_previous_budget = $this->get_immediate_previous_budget($office_id,$current_budget_fy,$header_id);

    $budget_tag_level_of_previous_budget = $immediate_previous_budget['budget_tag_level'];

    if($budget_tag_level_of_previous_budget > 0 ){

      $previous_budget_id = $immediate_previous_budget['budget_id'];
